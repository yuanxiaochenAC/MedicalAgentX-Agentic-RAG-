...
representation.Ensure that all the prompts required by the current graph from prompt are included.Exclude any other prompts.
Output the modified graph and all the necessary Prompts in prompt (if needed).
The prompt you need to generate is only the one used in `prompt.XXX` within Custom. Other methods already have built-in prompts and are prohibited from being generated. Only generate those needed for use in `prompt_custom`; please remove any unused prompts in prompt_custom.
...
WORKFLOW_TEMPLATE = """import evoagentx.workflow.operators as operator
import examples.aflow.humaneval.prompt as prompt_custom
